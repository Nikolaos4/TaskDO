...
func CreateUser(c *gin.Context) {
	...
	user_data := schemas.ResponseUserData{
		ID: data.ID,
		Login: data.Login,
...
func LoginUser(c *gin.Context) {
	...
	current_user := db.User{Login: req.Login} 
	if err := db.DB.First(&current_user).Error; err != nil {
		schemas.ResponseJSON(c, http.StatusNotFound, "User is not found", err)
		return
	}

	if !(security.Password_checking(current_user.Password, req.Password)) {
		schemas.ResponseJSON(c, http.StatusBadRequest, "Invalid password", nil)
		return
	}
